...
public class ParameterNamePattern implements IExpressionPattern<Cell> {
    private Pattern m_pattern = Pattern.compile("\\{parameter(\\[([0-9]+)\\])?\\.name\\}");

    public ParameterNamePattern() {
    }
    ...
    public boolean match(Context<Cell> context, ProgramElementDoc ped, Cell target, String value) {
        ...
        if ((ped instanceof ExecutableMemberDoc)) {
            ...
            if (index < parameters.length) {
                target.setCellValue(parameters[index].name());
                return true;
            }
        }
 ...
 * jp.co.shin_gi.javadoc.excel.ParameterNamePattern Java Class Version: 7 (51.0) JD-Core Version: 0.7.0.1
 */