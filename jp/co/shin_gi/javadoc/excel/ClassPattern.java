...
public class ClassPattern implements IExpressionPattern<Cell> {
    private Pattern m_pattern = Pattern.compile("\\{class\\}");
    ...
    public boolean match(Context<Cell> context, ProgramElementDoc ped, Cell target, String value) {
        ...
        if ((ped instanceof ClassDoc)) {
            ...
            target.setCellValue(classDoc.name());
        } else if ((ped instanceof MemberDoc)) {
            MemberDoc memberDoc = (MemberDoc) ped;
            target.setCellValue(memberDoc.containingClass().name());
        }
        return true;
    }
}
 ...
 * jp.co.shin_gi.javadoc.excel.ClassPattern Java Class Version: 7 (51.0) JD-Core Version: 0.7.0.1
 */